...
export interface StickersClient {
  /**
   * Provided a sticker pack ID and key, queries the Talk API and resolves
   * with a sticker pack manifest.
   */
  getStickerPackManifest(id: string, key: string): Promise<StickerPackManifest>;

  /**
   * Provided a sticker pack ID, its key, and a sticker ID, queries the Talk
...
export default function StickersClientFactory(options: StickersClientOptions): StickersClient {
   ...
   * @private
   *
   * gRPC type definition for Talk's sticker pack manifests.
   */
   ...
   * @private
   *
   * Provided a key and an encrypted manifest from the Talk API, resolves with
